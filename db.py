...
def por_genero(conexao, txt):
    filme = None
    try:
        var_select = "SELECT titulo, genero.nome, ano from filme, genero WHERE genero.nome = '"+ txt +"' AND id_genero = genero.id"
        cursor = conexao.cursor()
        cursor.execute(var_select)
        filme = cursor.fetchall()
    except sqlite3.Error as e:
...
def por_ano(conexao, txt):
    filme = None
    try:
        var_select = "SELECT titulo, genero.nome, ano from filme, genero WHERE ano = "+ txt +" AND id_genero = genero.id"
        cursor = conexao.cursor()
        cursor.execute(var_select)
        filme = cursor.fetchall()
    except sqlite3.Error as e:
